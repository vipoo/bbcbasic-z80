...
;
	...
	PUBLIC	TKN_FIRSTCMD
	EXTERN	TKE_DECODE, TKE_TOPV, TK_EXTENDED, TKX_EXEC_FIRST
	EXTERN	TKR_ELSE, TKR_OFF, TKR_ERROR, TKR_STEP, TKR_LINE, TKR_THEN, TKR_TAB, TKR_SPC, TKR_OR, TKR_AND, TKR_TAB, TKR_SPC
;
;
;
#define TKN_VECTOR()      				!
...
CMDTAB:
	TKN_VECTOR($C6,AUTO)
	TKN_VECTOR($C7,DELETE)
	TKN_VECTOR($C8,LOAD)
	TKN_VECTOR($C9,LIST)
	TKN_VECTOR($CA,NEW)
	TKN_VECTOR($CB,OLD)
	TKN_VECTOR($CC,RENUMBER)
	TKN_VECTOR($CD,SAVE)
	TKN_VECTOR($CE,PUT)
	TKN_VECTOR($CF,PTR)
	TKN_VECTOR($D0,PAGEV)
	TKN_VECTOR($D1,TIMEV)
	TKN_VECTOR($D2,LOMEMV)
	TKN_VECTOR($D3,HIMEMV)
	TKN_VECTOR($D4,SOUND)
	TKN_VECTOR($D5,BPUT)
	TKN_VECTOR($D6,CALL)
	TKN_VECTOR($D7,CHAIN)
	...
	TKN_VECTOR($DA,CLG)
	TKN_VECTOR($DB,CLS)
	TKN_VECTOR_DIS($DC,DATA,REM)
	TKN_VECTOR_DIS($DD,DEF,REM)
	TKN_VECTOR($DE,DIM)
	TKN_VECTOR($DF,DRAW)
	TKN_VECTOR($E0,END)
	TKN_VECTOR($E1,ENDPRO)
	TKN_VECTOR($E2,ENVEL)
	TKN_VECTOR($E3,FOR)
	TKN_VECTOR($E4,GOSUB)
	TKN_VECTOR($E5,GOTO)
	...
	TKN_VECTOR($F3,READ)
	TKN_VECTOR($F4,REM)
	TKN_VECTOR($F5,REPEAT)
	TKN_VECTOR($F6,REPOR)
	TKN_VECTOR($F7,RESTOR)
	TKN_VECTOR($F8,RETURN)
	TKN_VECTOR($F9,RUN)
	TKN_VECTOR($FA,STOP)
	TKN_VECTOR($FB,COLOUR)
	TKN_VECTOR($FC,TRACE)
	TKN_VECTOR($FD,UNTIL)
	TKN_VECTOR($FE,WIDTHV)

; $FF RESERVED FOR TK_EXTENDED
; Extended Tokens
	TKN_VECTOR('A',CLI)
	TKN_VECTOR('B',PORTOUT)

TKN_FIRSTCMD	EQU	TKN_AUTO

PORTOUT:
	CALL	EXPRI			; PORT ADDR
	EXX
	PUSH	HL
	CALL	COMMA
	CALL	EXPRI			; PORT VALUE
	EXX
	LD	A, L
	POP	BC
	OUT	(C), A
...
XEQ1:	CALL	NXT
	...
	CP	TK_EXTENDED - TKN_FIRSTCMD
	JR	NZ, EXQ_NOT_EXTENDED

	PUSH	AF		; RETRIEVE AND APPLY EXTENDED TOKEN OFFSET
	LD	A, (IY)
	SUB	TKX_EXEC_FIRST
	LD	B, A
	INC	IY
...
;[LET] var = expr
;
LET0:	CP	TKR_ELSE-TKN_FIRSTCMD
	JR	Z,REM
	CP	'*'-TKN_FIRSTCMD+100H
	JR	Z,EXT
	CP	'='-TKN_FIRSTCMD+100H
	JR	Z,FNEND
	CP	'['-TKN_FIRSTCMD
...
; PROCFRED(A,B)    TKN_DEF PROCFRED(B,A)     WORK.
