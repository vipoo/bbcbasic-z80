...
;
	...
	EXTERN	TKN_FIRSTCMD, TKR_AND, TKR_DIV, TKR_EOR, TKR_MOD, TKR_OR, TK_EXTENDED, TKX_EVAL_FIRST
;
...
TKE_FIRST	EQU	TKE_DECODE
	TKE_VECTOR($8D,DECODE)
	TKE_VECTOR($8E,OPENIN)
	TKE_VECTOR($8F,PTR)
	TKE_VECTOR($90,PAGEV)
	TKE_VECTOR($91,TIMEV)
	TKE_VECTOR($92,LOMEMV)
	TKE_VECTOR($93,HIMEMV)
	TKE_VECTOR($94,ABS)
	TKE_VECTOR($95,ACS)
	TKE_VECTOR($96,ADVAL)
	TKE_VECTOR($97,ASC)
	...
	TKE_VECTOR($AB,LOG)
	TKE_VECTOR($AC,NOT)
	TKE_VECTOR($AD,OPENUP)
	TKE_VECTOR($AE,OPENOT)
	TKE_VECTOR($AF,PI)
	TKE_VECTOR($B0,POINT)
	TKE_VECTOR($B1,POS)
	TKE_VECTOR($B2,RAD)
	...
	TKE_VECTOR($BD,CHRS)
	TKE_VECTOR($BE,GETS)
	TKE_VECTOR($BF,INKEYS)
	TKE_VECTOR($C0,LEFTS)
	TKE_VECTOR($C1,MIDS)
	TKE_VECTOR($C2,RIGHTS)
	TKE_VECTOR($C3,STRS)
	TKE_VECTOR($C4,STRING)
	TKE_VECTOR($C5,EOF)

; Extended Tokens
	TKE_VECTOR('a',INPORT)

TKE_EXTOFFSET	EQU	TKE_EOF+1		; THE NUMBER OF NORMAL EVAL TOKEN VECTORS
TKE_FIRSTCODE	EQU	'a'			; THE EXTENDED TOKEN FIRST VALUE
...
EXPR0A:	CP	TKR_EOR		;CHECK OPERATOR
	JR	Z,EXPR0B
	CP	TKR_OR
	RET	NZ
...
ITEM:	CALL	CHECK
	...
	LD	A, (IY)
	INC	IY
	SUB	TKE_FIRSTCODE-TKE_EXTOFFSET
	JP	DISPAT
