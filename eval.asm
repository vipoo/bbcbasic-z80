...
;
	...
	EXTERN	TKN_FIRSTCMD, TK_TAND, TK_TDIV, TK_TEOR, TK_TMOD, TK_TOR, TK_EXTENDED, TKX_EVAL_FIRST
;
;TABLE OF ADDRESSES FOR FUNCTIONS:
;
TKE_FIRST	EQU	TKE_DECODE

#define TKE_VECTOR()      				!
	PUBLIC	TKE_#1					!
TKE_#1:	EQU	TKN_FIRSTCMD - (FUNTBLEND - $) / 2	!
	DEFW	#1

#define TKE_VECTOR_DIS()      				!
	PUBLIC	TKE_#1					!
TKE_#1:	EQU	TKN_FIRSTCMD - (FUNTBLEND - $) / 2	!
	DEFW	#2

FUNTBL:
	TKE_VECTOR(DECODE)	;Line number ; 8D
	TKE_VECTOR(OPENIN)	;OPENIN 8E
	TKE_VECTOR(PTR)		;PTR	8F
	TKE_VECTOR(PAGEV)	;PAGE	90
	TKE_VECTOR(TIMEV)	;TIME	91
	TKE_VECTOR(LOMEMV)	;LOMEM
	TKE_VECTOR(HIMEMV)	;HIMEM
	TKE_VECTOR(ABS)		;ABS
	TKE_VECTOR(ACS)		;ACS
	TKE_VECTOR(ADVAL)	;ADVAL
	TKE_VECTOR(ASC)		;ASC
	TKE_VECTOR(ASN)		;ASN
	TKE_VECTOR(ATN)		;ATN
	TKE_VECTOR(BGET)	;BGET
	TKE_VECTOR(COS)		;COS
	TKE_VECTOR(COUNTV)	;COUNT
	TKE_VECTOR(DEG)		;DEG
	TKE_VECTOR(ERLV)	;ERL
	TKE_VECTOR(ERRV)	;ERR
	TKE_VECTOR(EVAL)	;EVAL
	TKE_VECTOR(EXP)		;EXP
	TKE_VECTOR(EXT)		;EXT
	TKE_VECTOR(ZERO)	;FALSE
	TKE_VECTOR(FN)		;FN
	TKE_VECTOR(GET)		;GET
	TKE_VECTOR(INKEY)	;INKEY
	TKE_VECTOR(INSTR)	;INSTR(
	TKE_VECTOR(INT)		;INT
	TKE_VECTOR(LEN)		;LEN
	TKE_VECTOR(LN)		;LN
	TKE_VECTOR(LOG)		;LOG
	TKE_VECTOR(NOT)		;NOT
	TKE_VECTOR(OPENUP)	;OPENUP
	TKE_VECTOR(OPENOT)	;OPENOUT
	TKE_VECTOR(PI)		;PI
	TKE_VECTOR(POINT)	;POINT(
	TKE_VECTOR(POS)		;POS
	TKE_VECTOR(RAD)		;RAD
	TKE_VECTOR(RND)		;RND
	TKE_VECTOR(SGN)		;SGN
	...
	TKE_VECTOR(SQR)		;SQR
	TKE_VECTOR(TAN)		;TAN
	TKE_VECTOR(TOPV)	;TO(P)
	TKE_VECTOR(TRUE)	;TRUE
	TKE_VECTOR(USR)		;USR
	TKE_VECTOR(VAL)		;VAL
	TKE_VECTOR(VPOS)	;VPOS
	TKE_VECTOR(CHRS)	;CHR$
	TKE_VECTOR(GETS)	;GET$
	TKE_VECTOR(INKEYS)	;INKEY$
	TKE_VECTOR(LEFTS)	;LEFT$(
	TKE_VECTOR(MIDS)	;MID$(
	TKE_VECTOR(RIGHTS)	;RIGHT$(
	TKE_VECTOR(STRS)
	TKE_VECTOR(STRING)
	TKE_VECTOR(EOF)		;EOF		;C5
	TKE_VECTOR(INPORT)
FUNTBLEND:

TKE_EXTOFFSET	EQU	TKE_INPORT

;
TCMD	EQU	TKE_FIRST + (FUNTBLEND - FUNTBL) / 2
;
...
EXPR0A:	CP	TK_TEOR		;CHECK OPERATOR
	JR	Z,EXPR0B
	CP	TK_TOR
	RET	NZ
...
ITEM:	CALL	CHECK
	...
	LD	A, (IY)
	INC	IY
	ADD	-TKX_EVAL_FIRST + TKE_EXTOFFSET
